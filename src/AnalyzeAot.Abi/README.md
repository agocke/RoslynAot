# AnalyzeAot native ABI

`AnalyzeAot.Abi` defines the private boundary between the NativeAOT compiler and
...
`analyze_aot_get_analyzer_module_v*` entry point. The returned
`IAnalyzerModule`:
