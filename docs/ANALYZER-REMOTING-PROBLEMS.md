...
### 25. Analyzer-constructed visitors dispatch analyzer-side

Discovered 2026-08-17 as CA1845's failure, closed 2026-08-18 except for its
ownership half.

`UseSpanBasedStringConcat.FlattenBinaryOperation` news up its own
`OperationWalker` subclass and calls `Visit` on a compiler-owned operation.
`OperationWalker` was projected as compiler-owned, so `Visit` was a remoted
member — and an analyzer-constructed instance has no handle behind it, so it
threw `InvalidOperationException: this Roslyn facade value has no control
vtbl`. That is a different failure class from the rest of the burn-down: not a
missing member, but a projected member reached on an object the projection does
not own.

The fix is to dispatch analyzer-side, in exactly Roslyn's own shape: `Visit`
resolves the operation's runtime type and calls the matching `VisitXxx` so an
analyzer's override wins; `VisitXxx` falls through to `DefaultVisit`;
`DefaultVisit` does nothing, or walks children. All of it is derivable from
public API — each `VisitXxx` method's single parameter names the interface to
dispatch on — so the 131 members are generated by one rule rather than declared
one at a time.

Three things this needed:

- **A most-derived runtime type query.** `IsObjectType` answers "is it this
  one" and needs a candidate; a switch over 129 operation interfaces would cost
  a crossing per candidate on every node. `GetObjectRuntimeVtblId` returns the
  answer in one crossing, cached compiler-side by runtime type.
- **A specificity order that includes interfaces.** Ranking by base-type chain
  alone puts every interface at one, because an interface has no base type,
  leaving the order among them alphabetical. Harmless for a yes/no query and
  wrong for a most-derived one, where it would answer `IOperation` for an
  `IBinaryOperation`. Adding the transitive interface count separates them.
- **`ToImmutableArray` for the child walk.** `IOperation.ChildOperations` is
  projected but `OperationList.Enumerator`'s `MoveNext` and `Current` are not,
  so `foreach` over the list itself would throw on the first child.

CA1845 passes. The whole-assembly module **shrank 78,832 bytes**: 129 remoted
`VisitXxx` bodies became one-line local calls, which is more than the dispatch
table costs. The floor grew 16 bytes.

**The ownership half is held back, and this is the interesting part.**
`OperationVisitor` and `OperationWalker` are still labelled `Remote` by
derivation, which is wrong — the compiler never constructs one. Declaring them
`Local`, which is the modelling-correct change, **regresses three rules from a
visible exception to a silent missing diagnostic**: CA1851, CA1865, and CA1866
stop throwing and stop reporting. Measured, not predicted — bisected by
reverting only the two ownership entries, which restores 14 passing with no
regression.

The obvious explanation is wrong. `Local` withdraws
`IOperation.Accept(OperationVisitor)` from the ABI, since an analyzer-owned
type cannot cross as a handle — but that member is called **zero** times across
the entire corpus, so it is not the mechanism. The cause is not yet understood.

It is held back deliberately. Shipping a change that converts three
`AD0001`s into silently absent diagnostics would trade the failure class this
project can see for the one it cannot, which is the trade problems 21 and 22
both argue against. The member-level fix does not depend on it: the members
that matter now have local bodies regardless of the type's label, and the only
thing the `Remote` label still permits is a transport for `Accept` that nothing
uses.
